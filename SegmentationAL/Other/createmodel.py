...
def createmodel():
    ...
    # CropLayer1 = layers.Cropping2D(cropping=((0,0),(0, 0)),
    ...
    ResizeLayer1 = layers.MaxPool2D(pool_size=(2, 2),
                                    strides=None,padding='valid',data_format=None,)(positive_input)
    
    ResizeLayer2 = layers.MaxPool2D(pool_size=(4, 4),
                                    strides=None,padding='valid',data_format=None,)(positive_input)
    
    
    
    ResizeLayer1 = layers.Resizing(scaleimsize,scaleimsize,
                                   interpolation='bilinear')(ResizeLayer1)
    
    ResizeLayer2 = layers.Resizing(scaleimsize,scaleimsize,
                                   interpolation='bilinear')(ResizeLayer2)
    
    
    out1 = cnn1(ResizeLayer1)
    out2 = cnn2(ResizeLayer2)
    
    # model = Model(positive_input,out1,name='Model_UL')
    ...
    # ResizeLayer1out = tf.keras.layers.Resizing(int(scaleimsize),int(scaleimsize),
    #                                            interpolation='bilinear')(emb1.output)
    
    # ResizeLayer2out = tf.keras.layers.Resizing(int(scaleimsize),int(scaleimsize),
    ...
    x = keras.layers.Add()([UNet(positive_input), out1,out2])
    # x = keras.layers.Concatenate()([UNet(positive_input), z3])
    
    x = layers.Conv2D(32, 1, padding='valid',activation="relu",name='eso')(x)
