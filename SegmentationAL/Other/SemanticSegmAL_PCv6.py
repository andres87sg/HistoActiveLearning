...
def TrainModel(train_generator,steps_train,valid_generator,steps_valid,load_weights):
    ...
    if load_weights==1:
        ...
        #tf.keras.Model.load_weights(path).expect_partial()
...
def TestingPool(img_path,mask_path,files,maskfiles,print_metrics_summary,path_model):
    ...
    for imfile,maskfile in zip(files,maskfiles):
        ...
        # Prediction
        predmask = np.squeeze(model.predict(np.expand_dims(imarray_resized,axis=0),verbose=0),axis=0)
        
        # Choose Lung Infection mask (Prediction and groundtruth)
        predinfectionmask = np.int16(predmask[:,:,0]>0.5) # Predicted mask
        trueinfectionmask = np.int16(mask_resized[:,:,2]//255) # Groundtruth mask
        
        # Flattening mask
        predinfectionmask_flat = predinfectionmask.flatten()
        trueinfectionmask_flat = trueinfectionmask.flatten()
        
        # Intersection and Union
        intersection = predinfectionmask_flat & trueinfectionmask_flat
        union = predinfectionmask_flat | trueinfectionmask_flat
        
        if np.sum(trueinfectionmask_flat)>0:
            
          delta=0.0001
          IoU =np.sum(intersection)/(np.sum(union)+delta)
          Dice = 2*np.sum(intersection)/(np.sum(predinfectionmask_flat)+np.sum(trueinfectionmask_flat)+delta)
...
path = 'C:/Users/Andres/Desktop/destino 17a/Testing/PC/'
maskpath = 'C:/Users/Andres/Desktop/destino 17a/Testing/PC_SG/'
...
for imfile,maskfile in zip(files,maskfiles):
    ...
    plt.subplot(1,3,2)
    plt.imshow(trueinfectionmask,cmap='gray')
    plt.axis('off')
    plt.title('Grountruth')
    
    plt.subplot(1,3,3)
    plt.imshow(predinfectionmask, cmap='gray')
    plt.axis('off')
